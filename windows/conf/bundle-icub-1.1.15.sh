export BUNDLE_ICUB_VERSION=1.1.15
...
_BUNDLE_YARP_DIR="F:\cygwin\home\icub\yarp-packaging-windows\build"
export BUNDLE_YARP_DIR=`cygpath $_BUNDLE_YARP_DIR`
export BUNDLE_CMAKE_PARAMETERS="-DENABLE_icubmod_cartesiancontrollerclient:BOOL=TRUE -DENABLE_icubmod_cartesiancontrollerserver:BOOL=TRUE -DENABLE_icubmod_gazecontrollerclient:BOOL=TRUE -DENABLE_icubmod_logpolarclient:BOOL=TRUE -DENABLE_icubmod_logpolarserver:BOOL=TRUE -DENABLE_icubmod_debugInterfaceClient:BOOL=TRUE -DENABLE_icubmod_fakecan=ON -DENABLE_icubmod_static_grabber=ON"