...
if [ ! -e ${ICUB_BUILD_CHROOT}/tmp/iCub-package.done ]; then
  ...
  if [ ! -e "${ICUB_BUILD_CHROOT}/${ICUB_INI_PATH}/${ICUB_INI_FILE}" ]
  then
    run_in_chroot "mkdir -p $ICUB_INI_PATH"
    run_in_chroot "ls  $ICUB_INI_PATH"
    run_in_chroot "touch ${ICUB_INI_PATH}/${ICUB_INI_FILE}"
    run_in_chroot "echo ###### This file is automatically generated by CMake. >> ${ICUB_INI_PATH}/${ICUB_INI_FILE}"
    run_in_chroot "echo [search iCub] >> ${ICUB_INI_PATH}/${ICUB_INI_FILE}"
    run_in_chroot "echo path "/usr/share/iCub">> ${ICUB_INI_PATH}/${ICUB_INI_FILE}"
  fi
...
Description: Software platform for iCub humanoid robot with simulator.
 ...
 interact with the real iCub robot, or with the included simulator." | sudo tee ${ICUB_BUILD_CHROOT}/${D_ICUB_INSTALL_DIR}/DEBIAN/control
  ...
  run_in_chroot "chown -R 1000:1000 $D_ICUB_INSTALL_DIR/usr/share/yarp"
  run_in_chroot "chmod -R g+w $D_ICUB_INSTALL_DIR/usr/share/iCub"
  run_in_chroot "chmod -R g+w $D_ICUB_INSTALL_DIR/usr/share/yarp"
