...
ICUB_DEPENDENCIES="libc6, python (<= 3), libncurses5-dev,  libglademm-2.4-dev, libqt3-mt-dev,  libcv-dev, libhighgui-dev, libcvaux-dev,  libsdl1.2-dev, subversion, gfortran, freeglut3-dev, cmake"  #Debian control file wants commas (,) to separate name of packages
BUILD_DEPENDENCIES="cmake wget unzip subversion gfortran libncurses5-dev  libglademm-2.4-dev libqt3-mt-dev libcv-dev libhighgui-dev libcvaux-dev freeglut3-dev libsdl1.2-dev"   	# apt-get do not wants commas
